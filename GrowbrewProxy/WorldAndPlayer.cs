...
namespace GrowbrewProxy
{
    ...
    public class World
    {
        ...
        private int readPos; // good to keep track of world serialization pos :)
        ...
        {
            foreach (Player pl in players.Where(pl => pl.name == name))
                return pl.netID;
            return -1;
        }
        ...
        {
            foreach (Player pl in players.Where(pl => pl.netID == netID))
        ...
        private Point GetPlayerBtnLocation()
        {
            ...
            {
                y += 32 * 2;
                if (y < PlayerForm.updatedHeight - 100) continue;
                y = 32;
                x += 86 * 2;
            }
        ...
        public void RemovePlayerControl(int netID)
        {
            ...
            for (int i = 0; i < playerForm.Controls.Count; i++)
            {
                ...
                if (control.Name != netID.ToString()) continue;
                MainForm.LogText += "[" + DateTime.UtcNow +
                                    "] (PROXY): Removed player from player control box with netID: " + netID + "\n";
        ...
        private void TileExtra_Serialize(byte[] dataPassed, int loc)
        {
            ...
            switch (a)
            {
                ...
                {
                    // this is door
                    len = BitConverter.ToInt16(dataPassed, readPos);
                    readPos += 2;
                    //string f = "";
                ...
                {
                    readPos++;
                    int c = BitConverter.ToInt32(dataPassed, readPos);
                    readPos += 4;
                    readPos += 4 * c;
                }
                    break;
                ...
                {
                    // storage box xtreme
                    short itemsSize = BitConverter.ToInt16(dataPassed, readPos);
                    readPos += 2;
                    readPos += itemsSize;
                }
                    break;
                ...
                {
                    // cybots
                    int r = BitConverter.ToInt32(dataPassed, readPos);
                    readPos += 4;
                    readPos += r * 15;
                    readPos += 8;
                }
        ...
        private int Tile_Serialize(byte[] dataPassed, int loc)
        {
            try
            {
                ...
                ushort fg = BitConverter.ToUInt16(dataPassed, readPos);
                readPos += 2; // sizeof short
                tiles[loc].fg = fg;
                ushort bg = BitConverter.ToUInt16(dataPassed, readPos);
                readPos += 2;
                tiles[loc].bg = bg;
                int add = BitConverter.ToInt32(dataPassed, readPos);
                tiles[loc].tileState = add;
                readPos += 4;
                if ((short) add > 0) readPos += 2;
        ...
        private int WorldObjects_Serialize(byte[] dataPassed)
        {
            try
            {
                dropped.id = BitConverter.ToInt16(dataPassed, readPos);
                readPos += 2;
                dropped.x = BitConverter.ToInt32(dataPassed, readPos);
                readPos += 4;
                dropped.y = BitConverter.ToInt32(dataPassed, readPos);
                readPos += 4;
                dropped.itemCount = dataPassed[readPos++];
                dropped.uid = BitConverter.ToInt32(dataPassed, readPos);
                readPos += 4;
                MainForm.LogText += "[" + DateTime.UtcNow + "] (PROXY): Serialized +1 dropped item object. ID: " +
                                    dropped.id + " X:" + dropped.x + " Y: " + dropped.y + " UID: " + dropped.uid + "\n";
            }
            catch (ArgumentException)
            {
                return -1;
        ...
        public World LoadMap(byte[] packet)
        {
            try
            {
                ...
                byte[]
                    data = VariantList
                        .get_extended_data(
                            packet); // agh, maybe puublically declare and use then like that but too late, too lazy  to refactor now.
                if (data.Length < 8192) return this;
                if (data.Length > 200000) return this; // 200kb too big not gonna do that...

                readPos += 6;
                short pLen = BitConverter.ToInt16(data, readPos);
                readPos += sizeof(short); // 2

                for (int i = 0; i < pLen; i++)
                    currentWorld += (char) data[readPos++];

                width = BitConverter.ToInt32(data, readPos);
                readPos += sizeof(int);
                height = BitConverter.ToInt32(data, readPos);
                readPos += sizeof(int);
                tileCount = BitConverter.ToInt32(data, readPos);
                readPos += sizeof(int);
                tiles = new Tile[tileCount]; // allocate exact tile data.

                MainForm.LogText += "[" + DateTime.UtcNow + "] (PROXY): Loading world: " + currentWorld +
                                    ", total tile count is: " + tileCount + "\n";
                if (!MainForm.serializeWorldsAdvanced)
                {
                    MainForm.LogText += "[" + DateTime.UtcNow +
                                        "] (PROXY): NOTE: Advanced world serialization was disabled, not continuing to load tiles.\n";
                    return this;
                }
                ...
                for (int i = 0; i < tileCount; i++)
                    if (Tile_Serialize(data, i) != 0)
                        break;
                MainForm.LogText += "[" + DateTime.UtcNow + "] (PROXY): [" + currentWorld + "]" +
                                    " Tiles properly serialized (without any errors): " + tilesProperlySerialized +
                                    "\n";
                if (readPos >= data.Length) return this;
                //weather n dropped items:
                droppedItems.Clear();
                dropped_ITEMUID = BitConverter.ToInt32(data, readPos);
                readPos += sizeof(int);
                int count = BitConverter.ToInt32(data, readPos);
                readPos += sizeof(int);
                ...
                MainForm.LogText += "[" + DateTime.UtcNow + "] (PROXY): Succeeded loading world '" + currentWorld +
                                    "'! Total bytes serialized: " + total_bytes_serialized +
                                    ",  dropped item uid count: " + count + "\n";
            }
            catch
            {
                return this;
