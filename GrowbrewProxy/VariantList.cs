    ...
    internal class VariantList
    {
        ...
        public static VarList GetCall(byte[] package)
        {
            ...
            for (int i = 0; i < argsTotal; i++)
            {
                ...
                byte index = package[pos];
                pos++; // pls dont bully sm
                byte type = package[pos];
                pos++;
                ...
                switch (type)
                {
                    case 1:
                    {
                        float vFloat = BitConverter.ToUInt32(package, pos);
                        pos += 4;
                        varList.functionArgs[index] = vFloat;
                        break;
                    }
                    case 2: // string
                        int strLen = BitConverter.ToInt32(package, pos);
                        pos += 4;
                        ...
                        if (index > 0)
                        {
                            ...
                            ) // exceptionary function, having it easier like this :)
                            {
                    ...
                    case 5: // uint
                        uint vUInt = BitConverter.ToUInt32(package, pos);
                        pos += 4;
                        varList.functionArgs[index] = vUInt;
                        break;

                    case 9
                        : // int (can hold negative values, of course they are always casted but its just a sign from the server that the value was intended to hold negative values as well)
                        int vInt = BitConverter.ToInt32(package, pos);
                        pos += 4;
